...
public class CollectionBestPractices {
    ...
    public static void immutabilityExample() {
        ...
        try { immutableList.add("D"); } catch (UnsupportedOperationException e) { log.info("Cannot modify immutable list"); }
    ...
    public static void nullHandlingExample() {
        ...
        try { List.of("A", null, "C"); } catch (NullPointerException e) { log.info("List.of() rejects nulls"); }
        List<String> arrayList = new ArrayList<>(); arrayList.add("A"); arrayList.add(null); arrayList.add("C");
        log.info("ArrayList allows nulls: {}", arrayList);
        Map<String,String> hashMap = new HashMap<>(); hashMap.put("key1","value1"); hashMap.put(null,"nullKey"); hashMap.put("key2",null);
        log.info("HashMap with nulls: {}", hashMap);
        try { Set<String> ts = new TreeSet<>(); ts.add(null); } catch (NullPointerException e) { log.info("TreeSet rejects nulls"); }
        Map<String,Integer> scores = new HashMap<>(); scores.put("Alice",100);
    ...
    public static void iterationExample() {
        ...
        try {
            for (String item : items) { if (item.equals("B")) items.remove(item); }
        } catch (ConcurrentModificationException e) { log.info("Foreach removal triggers CME"); }
    ...
    public static void performanceExample() {
        ...
        int size = 10_000;
        List<Integer> arrayList = new ArrayList<>(); for (int i=0;i<size;i++) arrayList.add(i);
        long start = System.nanoTime(); long sum=0; for (int i=0;i<size;i++) sum += arrayList.get(i); long arrayNs = System.nanoTime()-start;

        List<Integer> linkedList = new LinkedList<>(); for (int i=0;i<size;i++) linkedList.add(i);
        start = System.nanoTime(); sum=0; for (int i=0;i<size;i++) sum += linkedList.get(i); long linkedNs = System.nanoTime()-start;
        log.info("Random access ArrayList={}ns LinkedList={}ns", arrayNs, linkedNs);

        Set<Integer> hashSet = new HashSet<>(); for (int i=0;i<size;i++) hashSet.add(i);
        start = System.nanoTime(); for (int i=0;i<size;i++) hashSet.contains(i); long hashNs = System.nanoTime()-start;
        Set<Integer> treeSet = new TreeSet<>(); for (int i=0;i<size;i++) treeSet.add(i);
        start = System.nanoTime(); for (int i=0;i<size;i++) treeSet.contains(i); long treeNs = System.nanoTime()-start;
    ...
    public static void commonPitfallsExample() {
        ...
        List<Integer> list1 = Arrays.asList(1,2,3); List<Integer> list2 = Arrays.asList(1,2,3);
        log.info("list1 == list2: {}", list1 == list2);
        log.info("list1.equals(list2): {}", list1.equals(list2));

        class BadPerson { String name; BadPerson(String n){name=n;} }
        Set<BadPerson> badSet = new HashSet<>(); badSet.add(new BadPerson("Alice")); badSet.add(new BadPerson("Alice"));
        log.info("BadPerson set size (expected 1 but got {}): {}", badSet.size(), badSet.size());

        class MutableKey { int value; MutableKey(int v){value=v;} @Override public int hashCode(){return value;} @Override public boolean equals(Object o){return o instanceof MutableKey mk && mk.value==value;} }
        Map<MutableKey,String> map = new HashMap<>(); MutableKey key = new MutableKey(1); map.put(key,"Value");
        log.info("Contains before mutation: {}", map.containsKey(key)); key.value=2; log.info("Contains after mutation: {}", map.containsKey(key));

        List<String> fixed = Arrays.asList("A","B","C");
        try { fixed.add("D"); } catch (UnsupportedOperationException e) { log.info("Cannot change size of Arrays.asList() list"); }
    }
